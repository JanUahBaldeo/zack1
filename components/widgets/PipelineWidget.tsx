...
export function PipelineWidget({ role }: PipelineWidgetProps) {
  ...
  const getStatusColor = (status: 'on-track' | 'delayed' | 'at-risk') => {
    switch (status) {
  ...
  const getLoansForStage = (stageName: string) => {
    return mockPipeline.filter(loan => loan.currentStage === stageName)
  }

  const getTotalValue = (loans: PipelineItem[]) => {
